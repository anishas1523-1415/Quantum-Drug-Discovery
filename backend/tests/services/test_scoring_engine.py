...
def test_full_data_candidate_gets_full_confidence(mock_dti, mock_mol):
    mock_dti.return_value = FULL_DTI
    mock_mol.return_value = FULL_MOL

    result = score_candidate(BASE_CANDIDATE)

    assert result["confidence"] == 1.0  # all four weighted components present
    assert set(result["component_scores"].keys()) == {"clinical_evidence", "dti_potency", "drug_likeness", "admet_safety"}
    assert result["composite_score"] > 0


@patch("services.scoring_engine.get_molecular_properties")
@patch("services.scoring_engine.get_bioactivity")
def test_biologic_scored_fairly_without_molecular_data(mock_dti, mock_mol):
    ...
    # Only clinical_evidence (weight 0.40) was available -> confidence
...
@patch("services.scoring_engine.get_molecular_properties")
@patch("services.scoring_engine.get_bioactivity")
def test_upstream_failures_degrade_confidence_not_crash(mock_dti, mock_mol):
    mock_dti.side_effect = DTIError("chembl down")
    mock_mol.side_effect = MolecularPropertiesError("chembl down")

    result = score_candidate(BASE_CANDIDATE)

    assert result["confidence"] == 0.40  # only clinical_evidence survived
...
@patch("services.scoring_engine.get_molecular_properties")
@patch("services.scoring_engine.get_bioactivity")
def test_score_candidates_ranks_highest_first(mock_dti, mock_mol):
...
@patch("services.scoring_engine.get_molecular_properties")
@patch("services.scoring_engine.get_bioactivity")
def test_explanation_only_mentions_available_signals(mock_dti, mock_mol):
    mock_dti.return_value = {"chembl_id": "X", "has_data": False, "record_count": 0, "best": None}
    mock_mol.return_value = BIOLOGIC_MOL
